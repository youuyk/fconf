...
if [ $master_mode -eq 1 ] ; then
    echo "running Hadoop $APPLICATION"
    start=`date +%s.%N`
    
    timeout $TIMEOUT docker exec -t ${spl_id}_spl_master bash -c "/hadoop/share/hadoop/mapreduce hadoop jar hadoop-mapreduce-examples-3.3.4.jar wordcount /opt/input.txt /opt/output"
    IS_TIMEOUT=$?
    finish=`date +%s.%N`
    echo "Done!!"
fi 
